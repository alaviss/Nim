...
proc runCI(cmd: string) =
  ...
  if coverage.len > 0:
    # The sources generated by csources might not match what generated by the
    # current compiler. As such it creates mismatches in the data. We clean that
    # up and redo the bootstrap with the current compiler to generate more
    # consistent coverage data.
    execFold(
      "Clear bootstrapping coverage data (gcda)",
      quoteShellCommand ["python3", "-m", "fastcov", "-z", "-d", "nimcache"]
    )
    execFold(
      "Clear bootstrapping coverage data (gcno)",
      quoteShellCommand ["python3", "-m", "fastcov", "-z", "-n", "-d", "nimcache"]
    )
    kochExecFold("Bootstrap with coverage", "boot -d:release " & coverage)

  ## build nimble early on to enable remainder to depend on it if needed
  kochExecFold("Build Nimble", "nimble " & coverage)
